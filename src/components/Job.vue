<template>
  <div class="background min-h-screen">
    <div class="container flex-column mt-8">
      ...
      <b-alert v-model="showReportAlert" variant="success" class="text-center" dismissible>
        ...
        <div class="relative mx-auto w-1/3">
          <div class="bg-white w-full rounded shadow-2xl flex flex-col p-5">
            ...
            <form class="bg-white rounded px-8 pt-6 pb-8 mb-4 text-center">
              ...
              <h4 class="pb-2 text-gray-600">This information will be used for your employer to choose you for this
                job. </h4>
              <div class="mb-4">
                <textarea
      ...
      <!--        Modal Report Job    -->
      <div v-if="reportMessageModal"
           class="fixed overflow-x-hidden overflow-y-auto inset-0 flex justify-center items-center z-50">
        <div class="relative mx-auto w-1/3">
          <div class="bg-white w-full rounded shadow-2xl flex flex-col p-5">
            <div class="text-2xl font-bold text-center">Report this job</div>
            <form class="bg-white rounded px-8 pt-6 pb-8 mb-4 text-center">
              <h2 class="text-xl pb-4">Please tell us why would you like to report the job?</h2>
              <h4 class="pb-2 text-gray-600">This information will be used to improve the content of the website. </h4>
              <div class="mb-4">
                <textarea
                    class="text-center shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    id="reportMessage" v-model="reportMessage" required rows="9" cols="50"
                    type="text" placeholder="..."></textarea>
              </div>
              ...
              <div class="flex items-center justify-between">
                <button
                    class="bg-red-400 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
                    @click="reportMessageModal = false" type="button">
                  Close
                </button>
                <button
                    @click="submitReportMessage()"
                    class="bg-purple-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline right"
                    type="button">
                  Submit
                </button>
      ...
      <div class="flex py-16 pb-0 ">
        <div class="w-6/12 text-xl rounded-3xl shadow p-16 pt-12 pb-20 mr-5 ">
          ...
          <div class="text-2xl text-center pb-4 font-bold "><h4
              class="rounded-2xl shadow-md inline-block p-2 px-4 bg-red-200">{{ job.payment }} <i
              class="fas fa-euro-sign "></i></h4></div>
          <div class="mt-5 p-2 py-4"><i class="fas fa-map-marker-alt"></i> {{ job.city }}, {{ job.address }}</div>
          ...
          <div class="mt-20 flex justify-end text-gray-500"><a class="underline" @click="addReportMessage" href="#">Report</a>
          </div>
        </div>
        <div class="w-5/12 mt-2 ml-24 rounded-3xl shadow-inner p-4 px-16 ">
          ...
          <div class="flex pt-4 px-4">
            <div class="text-xl text-center pb-4 font-bold text-gray-900 "><h4
                class="rounded-full h-16 w-20 shadow-xl inline-block py-3  bg-red-300">{{ user.jobs.length }} </h4>
            </div>
            <h4 class="text-center text-xl text-gray-900 p-3 px-4 ">Jobs posted</h4>
          </div>
          <div class="flex py-2 px-4">
            <div class="text-xl text-center pb-4 font-bold text-gray-900 "><h4
                class="rounded-full h-16 w-20 shadow-xl inline-block py-3  bg-red-400">{{ countPayment() }} </h4></div>
            <h4 class="text-center text-xl text-gray-900 p-3 px-4">Total euro payed</h4>
          </div>
          <div class="flex py-2 px-4">
            <div class="text-xl text-center pb-4 font-bold text-gray-900 "><h4
                class="rounded-full h-16 w-20 shadow-xl inline-block py-3  bg-red-500">Email </h4></div>
            <h4 class="text-center text-xl text-gray-900 p-3 px-4">{{ user.email }}</h4>
          </div>
        </div>
